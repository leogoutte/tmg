...
def fig(theta, tAA, tAB, valley):
    ...
    AllK, E = EnergiesOnPath(theta, valley)

    # trim energies
    Ks, Es = TrimEnergies(AllK,E,lim)

    # denisty of states
    eDOS, DOS = DensityOfStates(Es)
...
def Energies(theta, kx, ky, valley):
...
def EnergiesOnPath(theta, valley):
    # ktheta
    kt = -qb[1]
...
def DensityOfStates(E):
    ...
    energies=np.linspace(-300,300,num=size_energies)
...
def Figure(Ks,Es,DOS,eDOS,lim):
    ...
    fig.update_yaxes(title_text="Energies", range=[-lim, lim], row=1, col=1)
    fig.update_yaxes(range=[-lim, lim], row=1, col=2)


    fig.update_layout(height=600, width=800, title_text="Spectrum and DOS", showlegend=False)
    fig.show()
